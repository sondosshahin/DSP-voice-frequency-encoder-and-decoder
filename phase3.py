...
from scipy.fft import fft  # Added import for fft
...
def get_frequencies(segment):
    ...
    for char, encoding_freqs in encoding_frequencies.items():
        ...
        lowcut = min(encoding_freqs)
        highcut = max(encoding_freqs)
        fs = Fs
        order = 2
        nyquist = 0.5 * fs
        low = lowcut / nyquist
...
def get_decoded_char(frequencies):
    if len(frequencies) < 3:
        return ' '  # Unable to determine character if less than 3 frequencies detected
